...
from transformers import (
    ...
    CLIPVisionModel, CLIPTextModel, CLIPProcessor,
    ViTModel, BertModel
)
...
CONFIG = {
    "medical_vlm_type": "bioviL",
    
    "vision_model": "microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224",
    "text_model_baseline": "microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224",
    ...
    "data_dir": ".",
    "checkpoint_dir": os.path.join(BASE_DIR, "vqa_medvlm_outputs", "checkpoints"),
    "checkpoint_path": "medvlm_radbert_best.pth",
    "results_dir": os.path.join(BASE_DIR, "vqa_medvlm_outputs", "results"),
    "vocab_path": os.path.join(BASE_DIR, "vqa_baseline_outputs", "checkpoints", "answer_vocab.json"),
...
class CoAttentionFusion(nn.Module):
    def __init__(self, hidden_dim=768, num_heads=8, dropout=0.1):
        ...
        self.ln_v1 = nn.LayerNorm(hidden_dim)
        self.ln_v2 = nn.LayerNorm(hidden_dim)
        self.ln_t1 = nn.LayerNorm(hidden_dim)
        self.ln_t2 = nn.LayerNorm(hidden_dim)
    
    def forward(self, vision_feats, text_feats):
        v_cross, _ = self.vision_to_text(
            query=vision_feats,
            key=text_feats,
            value=text_feats
        )
        vision_feats = self.ln_v1(vision_feats + v_cross)
        
        t_cross, _ = self.text_to_vision(
            query=text_feats,
            key=vision_feats,
            value=vision_feats
        )
        text_feats = self.ln_t1(text_feats + t_cross)
        
        v_ffn = self.vision_ffn(vision_feats)
        vision_feats = self.ln_v2(vision_feats + v_ffn)
        
        t_ffn = self.text_ffn(text_feats)
        text_feats = self.ln_t2(text_feats + t_ffn)
        
        return vision_feats, text_feats

class MultiLayerFusion(nn.Module):
    def __init__(self, num_layers=4, hidden_dim=768, num_heads=8, dropout=0.1):
        super().__init__()
        self.layers = nn.ModuleList([
            CoAttentionFusion(hidden_dim, num_heads, dropout)
...
class MedicalVLMWithSpecialist(nn.Module):
    def __init__(
        self,
        vision_encoder,
        text_encoder,
        ...
        if freeze_vision:
            ...
            print(f"  ✓ Medical vision encoder frozen (controlled experiment)")
...
class VQAMedDatasetMedVLM(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            if 'token_type_ids' in text_inputs:
                result['token_type_ids'] = text_inputs['token_type_ids'].squeeze(0)
...
def load_medical_vision_encoder(model_type, cache_dir):
    print(f"  Loading medical vision encoder: {model_type}")
    
    if "BiomedCLIP" in model_type or "biovil" in model_type.lower():
        try:
            ...
            print(f"  ✓ Loaded BiomedCLIP vision encoder")
            return vision_model
        except Exception as e:
            print(f"  ⚠ Could not load BiomedCLIP: {e}")
            print(f"  → Falling back to general ViT with medical fine-tuning recommended")
            vision_model = ViTModel.from_pretrained("google/vit-base-patch16-224", cache_dir=cache_dir)
            return vision_model
    else:
        vision_model = ViTModel.from_pretrained("google/vit-base-patch16-224", cache_dir=cache_dir)
        print(f"  ✓ Loaded ViT encoder (consider using medical pre-trained for best results)")
        return vision_model

def load_text_encoder(encoder_type, cache_dir):
    print(f"  Loading text encoder: {encoder_type}")
    
    if encoder_type == "clinical":
        model = AutoModel.from_pretrained(
            "emilyalsentzer/Bio_ClinicalBERT",
            cache_dir=cache_dir
        )
        print(f"  ✓ Loaded ClinicalBERT")
    elif encoder_type == "radbert":
        model = AutoModel.from_pretrained(
            "zzxslp/RadBERT-RoBERTa-4m",
            cache_dir=cache_dir
        )
        print(f"  ✓ Loaded RadBERT")
    elif encoder_type == "baseline_medical":
        try:
            model = AutoModel.from_pretrained(
                "microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224",
                cache_dir=cache_dir
            )
            print(f"  ✓ Loaded BiomedCLIP text encoder (PubMedBERT)")
        except:
            model = BertModel.from_pretrained("bert-base-uncased", cache_dir=cache_dir)
            print(f"  ✓ Loaded BERT baseline")
    else:
        model = BertModel.from_pretrained("bert-base-uncased", cache_dir=cache_dir)
        print(f"  ✓ Loaded BERT baseline")
...
def plot_final_training_curves(history, save_dir):
    ...
    ax = axes[0]
    ax.plot(epochs, train_losses, 'o-', label='Train Loss', linewidth=2, markersize=6, color='#e74c3c')
    ax.plot(epochs, val_losses, 's-', label='Val Loss', linewidth=2, markersize=6, color='#3498db')
    ...
    ax = axes[1]
    ax.plot(epochs, train_accs, 'o-', label='Train Accuracy', linewidth=2, markersize=6, color='#e74c3c')
    ax.plot(epochs, val_accs, 's-', label='Val Accuracy', linewidth=2, markersize=6, color='#3498db')
    ax.set_xlabel('Epoch', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'training_curves_medvlm.png'), dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"  ✓ Training curves saved to {os.path.join(save_dir, 'training_curves_medvlm.png')}")

def create_generalization_analysis(results_dict, save_dir):
    os.makedirs(save_dir, exist_ok=True)
    ...
    baseline_accs = [results_dict[arch]['baseline'] for arch in architectures]
    clinical_accs = [results_dict[arch]['clinical'] for arch in architectures]
    radbert_accs = [results_dict[arch]['radbert'] for arch in architectures]
    ...
    ax.set_title('Absolute Performance Across Architectures', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels([a.upper() for a in architectures], fontsize=11)
    ax.legend(fontsize=11)
    ax.set_ylim([0.6, 0.8])
    ...
    ax = axes[1]
    clinical_improvements = [clinical_accs[i] - baseline_accs[i] for i in range(len(architectures))]
    radbert_improvements = [radbert_accs[i] - baseline_accs[i] for i in range(len(architectures))]
    
    bars1 = ax.bar(x - width/2, clinical_improvements, width, label='ClinicalBERT Gain',
                   color='#e74c3c', edgecolor='black', linewidth=1.5, alpha=0.8)
    bars2 = ax.bar(x + width/2, radbert_improvements, width, label='RadBERT Gain',
                   color='#3498db', edgecolor='black', linewidth=1.5, alpha=0.8)
    
    ax.axhline(y=0, color='black', linestyle='-', linewidth=1)
    ax.set_ylabel('Improvement over Baseline', fontsize=13, fontweight='bold')
    ax.set_xlabel('Architecture', fontsize=13, fontweight='bold')
    ax.set_title('Specialization Benefit Across Architectures', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels([a.upper() for a in architectures], fontsize=11)
    ax.legend(fontsize=11)
    ax.grid(axis='y', alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'architecture_generalization.png'), dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"  ✓ Generalization analysis saved")
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        print(f"✓ TF32 enabled for faster training on RTX 4080 Super")
    ...
    print(f"\n{'='*70}")
    print(f"Medical VLM with Specialist Text Encoder")
    print(f"{'='*70}\n")
    print(f"Device: {device}")
    print(f"Vision: Medical pre-trained (frozen={CONFIG['freeze_vision']})")
    print(f"Text: {CONFIG['text_encoder_type']}")
    print(f"Fusion: {CONFIG['num_fusion_layers']} co-attention layers")
    ...
    print("Loading processors and encoders...")
    ...
    train_dataloader = DataLoader(
        ...
        collate_fn=custom_collate_fn, persistent_workers=True
    )
    ...
    vision_encoder = load_medical_vision_encoder(CONFIG['vision_model'], CONFIG['cache_dir'])
    text_encoder = load_text_encoder(CONFIG['text_encoder_type'], CONFIG['cache_dir'])
    
    print("\n  Creating medical VLM...")
    
    model = MedicalVLMWithSpecialist(
    ...
    print(f"  Optimizer: Text encoder (lr={CONFIG['lr']}) + Fusion (lr={CONFIG['fusion_lr']})")
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        if (epoch + 1) % 5 == 0 and epoch < CONFIG['epochs'] - 1:
            user_input = input(f"\n[Epoch {epoch+1}] Continue training? (y/n, default=y): ").strip().lower()
            if user_input == 'n':
    ...
    summary = {
        'model': f'MedicalVLM-{CONFIG["text_encoder_type"]}',
        'vision_encoder': 'Medical pre-trained',
        'text_encoder': CONFIG['text_encoder_type'],
        'best_val_accuracy': best_accuracy,
        'total_epochs': len(history),
        'architecture': 'Medical VLM with specialist text encoder',
        'config': CONFIG
    }
    ...
    plot_final_training_curves(history, CONFIG['results_dir'])
    
    print(f"\n{'='*70}")
    print(f"Training Complete!")
    print(f"{'='*70}")
    print(f"Architecture: Medical VLM")
    print(f"Vision: Medical pre-trained (frozen)")
    print(f"Text: {CONFIG['text_encoder_type']}")
