...
CONFIG = {
    ...
    "data_dir": BASE_DIR,
    "checkpoint_dir": os.path.join(BASE_DIR, "vqa_vilt_clinicalbert_outputs", "checkpoints"),
    "checkpoint_path": "vilt_clinicalbert_best.pth",
    "results_dir": os.path.join(BASE_DIR, "vqa_vilt_clinicalbert_outputs", "results"),
    "plots_dir": os.path.join(BASE_DIR, "vqa_vilt_clinicalbert_outputs", "plots"),
...
class ViltWithClinicalBERT(torch.nn.Module):
    def __init__(self, vilt_model, clinical_bert, num_labels, freeze_vision=True):
        super().__init__()
        ...
        print("  ✓ ClinicalBERT: Complete text encoder (embeddings + 12 layers)")
    
    def forward(self, input_ids, attention_mask, token_type_ids, pixel_values, pixel_mask):
        device = input_ids.device
        
        text_embeds = self.clinical_embeddings(
            input_ids=input_ids,
        ...
        text_encoded = clinical_outputs.last_hidden_state
...
def evaluate(model, dataloader, device):
    ...
    return {'loss': total_loss / len(dataloader), 'accuracy': correct / total, 'correct': correct, 'total': total}

def plot_final_training_curves(history, save_dir, model_name="clinicalbert"):
...
def save_complete_checkpoint(model, optimizer, scaler, epoch, metrics, config, vocab_dict, checkpoint_path):
    checkpoint = {
        ...
        'model_config': {
            ...
            'clinical_bert_name': config['clinical_bert_name'],
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"ViLT-ClinicalBERT Training - RTX 4080 Super Optimized")
    print(f"{'='*70}\n")
    ...
    vilt_model = ViltForQuestionAnswering.from_pretrained(CONFIG['model_name'], config=vilt_config, ignore_mismatched_sizes=True)
    ...
    clinical_bert = AutoModel.from_pretrained(CONFIG['clinical_bert_name'])
    
    print("  Creating hybrid model...")
    model = ViltWithClinicalBERT(vilt_model, clinical_bert, num_labels, CONFIG['freeze_vision_encoder']).to(device)
    ...
    history_path = os.path.join(CONFIG['results_dir'], 'training_history_clinicalbert.json')
    with open(history_path, 'w') as f:
        json.dump(history, f, indent=2)
    
    plots_dir = CONFIG.get('plots_dir', CONFIG['results_dir'])
    os.makedirs(plots_dir, exist_ok=True)
    plot_final_training_curves(history, plots_dir, "clinicalbert")
